...
public class JWTAuthenticationFilter  implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext request) throws IOException {
        if (isSecuredResource()) {
            ...
            if (token == null) {
                request.abortWith(exception.CarondoException.makeErrRes("Missing token", token, 403));
                return;
            }
            try {
                ...
                request.abortWith(exception.CarondoException.makeErrRes("Invalid token", "Token not valid (timed out?)", 403));
            }
        }
    }
    ...
    private UserPrincipal getUserPrincipalFromTokenIfValid(String token)
            throws ParseException, JOSEException, AuthenticationException, java.text.ParseException {
